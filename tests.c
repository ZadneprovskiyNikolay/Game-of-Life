...
void write_image(const char* file_name, const char* dir_name, byte* field, uint32 width, uint32 height) {
    ...
    struct stat st = {0};
    if (dir_name != NULL && stat(dir_name, &st) == -1) {
        if (dir_name != NULL && mkdir(dir_name, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH)) {            
...
int main(int argc, char* argv[]) {    
    ...
    write_image(file_name, NULL, field, width, height);    

    FILE* file1 = fopen("monochrome.bmp", "rb");
    FILE* file2 = fopen("output.bmp", "rb");
    
    if (files_equal(file1, file2)) {
        printf("PASSED");
    }
    else {
        printf("FAILED: rewrite equal .bmp file.\n");
        return 1;
    }
